...
import { getPicksForRace, savePickForRace } from '../utils/storage'
...
export default function PickEntry({ players }) {
  ...
  // Load existing pick when player or race changes
  useEffect(() => {
    if (!player || !raceId) return
    const picks = getPicksForRace(raceId)
    if (picks[player]) {
      setOrder([...picks[player]])
    } else {
  ...
  function handleSave() {
    if (!player) return
    savePickForRace(raceId, player, order)
    setSaved(true)
      ...
      {!player ? (
        ...
        <>
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            {saved && (
              <span className="text-green-400 text-sm">Pick saved!</span>
            )}
          </div>
        </>
      )}
